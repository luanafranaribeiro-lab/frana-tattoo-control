...
function tattooHasMimo(t){return(t.materials||[]).some(x=>{let s=db.stock.find(a=>String(a.id)===String(x.stockId));return !!s?.isMimoReady})}
function tattooMimoLine(t){return tattooHasMimo(t)?'<div class="tiny">Mimo: 1 un</div>':''}
function tattooHistoryRegularCard(t){
  let profit=(+t.price||0)-(+t.cost||0);
  return `<button class="card full-card" onclick="tattooSheet(${t.id})"><div class="row"><div><b>${esc(t.client||t.name||'Tattoo')}</b><div class="tiny">${esc(t.date||t.isoDate||'')}</div></div><b>${money(t.price)}</b></div><div class="tiny">Custo materiais ${money(t.cost)} · Lucro ${money(profit)}</div>${tattooMimoLine(t)}</button>`;
}
function agendaView(){
...
function tattooSheet(id){
  ...
  if(t.eventId!=null){let e=db.events.find(x=>String(x.id)===String(t.eventId)),rec=t.received==null?t.price:t.received;form(esc(t.client||t.name||'Flash'),`<div class="card">${e?`<div class="tiny">${esc(e.name)} · ${esc(e.date||t.date||'')}</div>`:''}<div class="row"><span>Recebido</span><b>${money(rec)}</b></div>${tattooMimoLine(t)}</div><div class="actions"><button id="editT">Editar</button><button id="delT">Excluir</button></div>`,closeM,'Fechar');$('#editT').onclick=()=>newTattoo(t);$('#delT').onclick=()=>{restore(t);db.tattoos=db.tattoos.filter(x=>x.id!==id);save();closeM();render()};return}
  let profit=(+t.price||0)-(+t.cost||0);
  form(esc(t.client||t.name||'Tattoo'),`<div class="card"><div class="row"><span>Valor</span><b>${money(t.price)}</b></div><div class="row"><span>Custo materiais</span><b>${money(t.cost)}</b></div><div class="row"><span>Lucro</span><b>${money(profit)}</b></div>${tattooMimoLine(t)}</div><div class="actions"><button id="editT">Editar</button><button id="delT">Excluir</button></div>`,closeM,'Fechar');$('#editT').onclick=()=>newTattoo(t);$('#delT').onclick=()=>{restore(t);db.tattoos=db.tattoos.filter(x=>x.id!==id);save();closeM();render()}
}